...
class Reportes extends CI_Controller {
	...
	/**
	 * Reporte accesos por cliente
	 *
	 * @author Ruben Guevara <rguevara@hotmail.es>
	 * @return [type] [description]
	 */
	public function excel_accesos_por_clientes()
	{
		ini_set('max_execution_time', 300);
	    ini_set('memory_limit','160M');
	    $allInputs = json_decode(trim($this->input->raw_input_stream),true);
	    ...
	    // SETEO DE VARIABLES
	    	...
	    	// $excel->setActiveSheetIndex(0);
			$sheet = $excel->getActiveSheet();
			$sheet->setTitle('Descargas');
			$sheet->setShowGridlines(false);
	    	$dataColumnsTP = array(
	    		array('col' => '#',			'ancho' => 7),
	    		array('col' => 'CODIGO SESION',	'ancho' => 15),
	    		array('col' => 'FECHA DE SESION',	'ancho' => 18),
	    		array('col' => 'HORA DE SESION',		'ancho' => 18),
	    		array('col' => 'DIRECCION IP',		'ancho' => 30),
	    	);
	    	$cantColumns = count($dataColumnsTP);
	    	$arrColumns = array();
	    	$a = 'B'; // INICIO DE COLUMNA
	    ...
	    // TITULO
			$sheet->setCellValue($arrColumns[0].'1', 'LISTADO DE ARCHIVOS DESCARGADOS');
			$sheet->getStyle($arrColumns[0].'1')->applyFromArray($styleArrayTitle);
			$sheet->mergeCells($arrColumns[0].'1:'. $endColum .'1');

			$sheet->getCell($arrColumns[0].'2')->setValue( 'CODIGO CLIENTE: '.$paramDatos['codigo']);
      		$sheet->getStyle($arrColumns[0].'2')->applyFromArray($styleArraySubTitle);
      		$sheet->mergeCells($arrColumns[0].'2:'. $endColum .'2');

      		$sheet->getCell($arrColumns[0].'3')->setValue( 'EXCURSION: '.$paramDatos['excursion'] . ' - ' . $paramDatos['fecha_excursion']);
      		$sheet->getStyle($arrColumns[0].'3')->applyFromArray($styleArraySubTitle);
      		$sheet->mergeCells($arrColumns[0].'3:'. $endColum .'3');

      		$currentCellEncabezado = 5; // donde inicia el encabezado del listado
	...
	/**
	 * Reporte Archivos descargados por cliente
	 *
	 * @author Ruben Guevara <rguevara@hotmail.es>
	 * [clientes_email_excel description]
	 * @return [type] [description]
	 */
	public function excel_descargas_por_clientes()
	{
		ini_set('max_execution_time', 300);
	    ini_set('memory_limit','160M');
	    $allInputs = json_decode(trim($this->input->raw_input_stream),true);
	    ...
	    // SETEO DE VARIABLES
	    	...
	    	$dataColumnsTP = array(
	    		array('col' => '#',			'ancho' => 7),
	    		array('col' => 'CODIGO ARCHIVO',	'ancho' => 15),
	    		array('col' => 'FECHA DE DESCARGA',	'ancho' => 18),
	    		array('col' => 'HORA DE DESCARGA',		'ancho' => 18),
	    		array('col' => 'NOMBRE DE ARCHIVO',		'ancho' => 60),
	    		array('col' => 'TAMAÑO DE ARCHIVO (MB)',		'ancho' => 20),
	    	);
	    	$cantColumns = count($dataColumnsTP);
	    	$arrColumns = array();
	    	$a = 'B'; // INICIO DE COLUMNA
