...
class Cliente extends CI_Controller {
    ...
    public function listar_clientes(){
		...
		foreach ($lista as $row) {
			...
			if( $this->sessionCI['key_grupo'] == 'key_root' || $this->sessionCI['key_grupo'] == 'key_admin'){
				$editar = TRUE;
			}else{
				$editar = FALSE;
			}
			...
			if(!empty( $_REQUEST )){
			    ...
			    if(in_array($file_ext,$extensions_image)){
			    	if($file_size < 13*_1MB){ //12MB (1 MB = 1048576)
				        ...
				        redimencionMarcaAgua(600, $file_tmp, $carpeta . DIRECTORY_SEPARATOR .'thumbs', $file_name);
	...
	/**
	 * Método para descomprimir un fichero zip, subido previamente via ftp,
	 * y organizar las imagenes segun el codigo de cliente en sus respectivas
	 * carpetas
	 * video: ddmmYYYY-idexcursion.mp4
	 *
	 * @return mixed
	 */
	public function organizar_imagenes_temporales()
	{
					...
					$rowCliente = $this->model_cliente->m_cargar_cliente_por_codigo($codigo);
		   			...
		   			array_push($arrPrincipal,
		   				array(
		   					...
		   					'deposito' => $rowCliente['deposito'],
		   					'precio_paquete' => $rowCliente['precio_paquete'],
		   					'procesado' => $rowCliente['procesado'],
		   				)
		   			);
				}
			}
		...
		// ORGANIZACION DEL FICHERO ZIP QUE CONTIENE LOS VIDEOS
		if(!empty($videosZip)){
			$error = FALSE;
			$i = 0;
			$zip = new ZipArchive;
	...
	public function cron_elimina_cliente()
	{
		$ajuste = $this->model_ajustes->m_cargar_ajuste_dias();
		$lista = $this->model_cliente->m_clientes_para_eliminar($ajuste['dias']);
		// var_dump($lista); exit();
		if(empty($lista)){
		...
		foreach ($lista as $row) {
			if( $this->model_archivo->m_delete_archivo($row) ){
				...
				if($this->model_cliente->m_anular_cliente($row)){
						echo $row['codigo'] . ' se anuló correctamente <br>';
				}
			}
		}
	}

	public function listar_acceso_clientes()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		if( empty($this->sessionCI) ){
			$arrData['flag'] = -1;
		...
		$paramPaginate = $allInputs['paginate'];
		$paramDatos = $allInputs['datos'];
		$lista = $this->model_cliente->m_listar_sesiones_cliente($paramPaginate,$paramDatos);
		if( !empty($lista) ){
			$totalRows = $this->model_cliente->m_count_sesiones_cliente($paramPaginate,$paramDatos);
		}
		$arrListado = array();
		$i = 1;
		foreach ($lista as $row) {
			array_push($arrListado, array(
				...
				'fecha_excursion' => darFormatoDMY($row['fecha_excursion']),
				'fecha' => darFormatoDMYHora($row['fecha']),
	...
	public function listar_detalle_acceso()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		if( empty($this->sessionCI) ){
			$arrData['flag'] = -1;
		...
		$paramPaginate = $allInputs['paginate'];
		$paramDatos = $allInputs['datos'];
		$lista = $this->model_cliente->m_listar_detalle_sesiones($paramPaginate,$paramDatos);

		$arrListado = array();
		foreach ($lista as $row) {
			array_push($arrListado, array(
				'idclientesesion' => $row['idclientesesion'],
				'fecha' => $row['fecha'],
				'hora' => $row['hora'],
				'ip' => $row['ip'],
				)
	...
	public function listar_detalle_descarga()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		if( empty($this->sessionCI) ){
			$arrData['flag'] = -1;
		...
		$paramPaginate = $allInputs['paginate'];
		$paramDatos = $allInputs['datos'];
		$lista = $this->model_cliente->m_listar_detalle_descarga($paramPaginate,$paramDatos);
	...
	/**
	 * Para cargar el pdf que muestra el listado de fotos descargadas de un cliente
	 *
	 * @return [type] [description]
	 */
	public function imprimir_accesos_por_clientes()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		$this->load->library('Fpdfext');
		$arrData = array();
		...
		$this->pdf->Cell(0,11,'',0,15);
		$this->pdf->Cell(0,7,utf8_decode('LISTADO DE SESIONES'),0,7,'C');
		$this->pdf->Ln(4);

		$this->pdf->SetFont('Arial','',11);
		$this->pdf->Cell(0,7,utf8_decode('CODIGO CLIENTE : ' . $paramDatos['codigo']));
		$this->pdf->Ln(6);
		$this->pdf->Cell(0,7,utf8_decode('EXCURSION : ' . $paramDatos['excursion']));
		$this->pdf->Ln(6);
		$this->pdf->Cell(0,7,utf8_decode('FECHA EXCURSION : ' . darFormatoDMY($paramDatos['fecha_excursion'])));
		$arrWidthCol = array(10,30,30,30,60); // ANCHO TOTAL: 190
        $arrHeaderText = array(
        	'Nº',
        	'CODIGO SESION',
        	'FECHA SESION',
        ...
        $arrHeaderAligns = array('C','C','C','C','C');
        $arrDataAligns = array('C','C','C','C','L');
	 ...
	 * @return [type] [description]
	 */
	public function imprimir_descargas_por_clientes()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		$this->load->library('Fpdfext');
		$arrData = array();
		...
		$this->pdf->Cell(0,11,'',0,15);
		$this->pdf->Cell(0,7,utf8_decode('LISTADO DE ARCHIVOS DESCARGADOS'),0,7,'C');
		$this->pdf->Ln(4);
		...
		$this->pdf->Ln(6);
		$this->pdf->Cell(0,7,utf8_decode('FECHA EXCURSION : ' . darFormatoDMY($paramDatos['fecha_excursion'])));
		$arrWidthCol = array(10,30,30,30,60,30); // ANCHO TOTAL: 190
        $arrHeaderText = array(
        	'Nº',
        	'CODIGO ARCHIVO',
        	'FECHA DESCARGA',
        	'HORA DESCARGA',
        	'NOMBRE ARCHIVO DESCARGADO',
        	'TAMAÑO (MB)'
        );
        $arrHeaderAligns = array('C','C','C','C','C','C');
        $arrDataAligns = array('C','C','C','C','L','R');
        $arrBoolMultiCell = array(0,0,0,0,0,0); // colocar 1 donde deseas utilizar multicell
        $countArray = count($arrWidthCol);
        $acumWidth = 0;
        $this->pdf->Ln(6);
        $this->pdf->SetFont('Arial','B',8);
        ...
        foreach ($lista as $row) {
        	...
        	$this->pdf->Row(
                array(
                    ...
                    $row['hora_descarga'],
                    $row['nombre_archivo'],
                    number_format($row['size']/1048576, 2)
                ),
            	$fill,1
            );
        }
